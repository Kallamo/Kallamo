...
// Resolve the string icon names from whatsNew.js to actual components.
const ICONS = { PenLine, Globe2, Brain, Sparkles };

// Each changelog group gets its own small icon + accent, purely presentational.
const SECTION_META = {
...
export default function WhatsNewModal({ onClose }) {
  const { version, title, intro, highlights, changelog } = WHATS_NEW;

  return (
    <Modal onClose={onClose} size="xl" className="max-h-[86vh]">
      {/* Hero banner — the onboarding lighthouse art, tying the release to the
          seeded example story and giving the modal a sense of place. */}
      <div className="relative shrink-0 h-40 overflow-hidden">
        <img src={heroBg} alt="" className="absolute inset-0 w-full h-full object-cover" />
        {/* Scrim so the title reads and the art melts into the modal body. */}
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col gap-0.5 pt-0.5">
                  ...
                  <p className="text-[0.8125rem] leading-relaxed text-gray-400">{h.text}</p>
                </div>
              </div>
            );
          })}
              ...
              return (
                <div key={section}>
                  <div className="flex items-center gap-2 mb-2">
                    <SIcon className={`w-3.5 h-3.5 ${meta.color}`} />
                  ...
                  <ul className="flex flex-col gap-1.5 pl-1 border-l border-gray-800/70 ml-1">
                    {items.map((it, i) => (
                      <li key={i} className="flex gap-2.5 text-[0.8125rem] leading-relaxed text-gray-400 pl-3.5">
