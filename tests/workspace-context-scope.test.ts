...
describe('workspace retrieval scope', () => {
  test('keeps workspace files shared or scoped to the executing profile', () => {
    const files = [
      ...
      { name: 'profile-1.txt', strategy: 'rag_search', profiles: JSON.stringify(['profile-1']) },
      { name: 'profile-2.txt', strategy: 'rag_search', profiles: ['profile-2'] },
      { name: 'disabled.txt', strategy: 'rag_search', enabled: false },
      { name: 'constant.txt', strategy: 'constant' }
    ];
    const results = files.map(file => ({
      source: file.name === 'profile-1.txt' ? 'PROFILE-1.TXT' : file.name,
      text: file.name
    }));
  ...
  test('keeps workspace memories shared or scoped to the executing profile', () => {
    const blocks = [
      ...
      { id: 'one', title: 'One', strategy: 'rag_search', profiles: JSON.stringify(['profile-1']) },
      { id: 'two', title: 'Two', strategy: 'rag_search', profiles: ['profile-2'] },
      { id: 'disabled', title: 'Disabled', strategy: 'rag_search', enabled: false },
